...
public class Neo4jTest {
	...
	public void testNeo4jRepository() {
		...
		List<Order> chucksOrders2 = orderRepository.findChucksOrders("Chuck Wagon");
		...
		// begin 混合自定义
		// Finding an order by a custom query method
		List<Order> siAOrders = orderRepository.findSiAOrders();
		assertEquals(1, siAOrders.size());
		assertEquals("Chuck Wagon", siAOrders.get(0).getCustomer());
		assertEquals(2, siAOrders.get(0).getItems().size());
		// end 混合自定义
